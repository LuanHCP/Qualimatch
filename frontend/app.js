...
async function loadContractorHome(){
 try{
  ...
  $('#contractor-kpis').innerHTML=[metric('Kits',d.kits,'Kits da base de demonstração',true),metric('Recebidos',d.received,'Entregas registradas como recebidas',true),metric('Status OK',d.ok,'Kits sem pendência',true),metric('Atenção',d.attention,'Pendentes ou incompletos',true),metric('Documentos',d.documents,'Arquivos indexados',true),metric('Vinculados',d.linked_documents,'Documentos associados a kits',true)].join('');
...
function accessLabel(v){return({ADMIN:'Administrador',EDITOR:'Editor',VIEWER:'Somente leitura'}[String(v||'').toUpperCase()]||v)}
function auditActionLabel(v){return({SYNC_DEMO:'Sincronização da demo',VIEW_CERTIFICATE:'Consulta de certificado',UPLOAD_KIT:'Envio de kit',UPDATE_TRACE:'Atualização de traço',CLOSE_NC:'Encerramento de NC'}[String(v||'').toUpperCase()]||String(v||'').replaceAll('_',' '))}
async function loadAdmin(){
 try{
  const [d,users]=await Promise.all([api('/api/admin/summary'),api('/api/admin/users')]);
  $('#admin-kpis').innerHTML=[metric('Usuários',d.users,'Perfis de demonstração',true),metric('Ativos',d.active_users,'Acesso liberado',true),metric('Bloqueados',d.blocked_users,'Acesso suspenso',true),metric('Eventos',d.audit_events,'Registros de auditoria',true),metric('Certificados',d.certificates,'Base de demonstração',true),metric('Kits',d.kits,'Base de demonstração',true),metric('Traços',d.traces,'Base de demonstração',true),metric('NCs',d.nonconformities,'Base de demonstração',true)].join('');
  $('#admin-users-body').innerHTML=users.map(x=>`<tr><td><b>${esc(x.name)}</b></td><td>${esc(x.email)}</td><td>${esc(x.role)}</td><td>${statusBadge(accessLabel(x.access_level))}</td><td>${statusBadge(x.status)}</td><td>${esc(x.last_access)}</td></tr>`).join('');
 }catch(e){toast(e.message,true)}
}
async function loadAdminHistory(){
 try{const list=await api('/api/admin/audit?limit=150');$('#admin-history-body').innerHTML=list.map(x=>`<tr><td>${esc(x.created_at)}</td><td><b>${esc(x.actor)}</b></td><td>${esc(auditActionLabel(x.action))}</td><td>${esc(x.target)}</td><td>${esc(x.detail)}</td></tr>`).join('')}catch(e){toast(e.message,true)}
}
